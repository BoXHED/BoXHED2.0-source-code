...
// BoXHED regression for survival data
class BoXHEDRegression : public ObjFunction {
 public:
  void Configure(
...
#pragma omp parallel for schedule(static)
  ...
  void SaveConfig(Json* p_out) const override {
    auto& out = *p_out;
    out["name"] = String("survival:boxhed");
  }
  void LoadConfig(Json const&) override {}
};

// register the objective function
XGBOOST_REGISTER_OBJECTIVE(BoXHEDRegression, "survival:boxhed")
.describe("BoXHED for time varying survival data")
.set_body([]() { return new BoXHEDRegression(); });



// BoXHED GPU regression for survival data
class BoXHEDRegression_GPU : public ObjFunction {
 public:
  void Configure(
      const std::vector<std::pair<std::string, std::string> > &args) override {}
  ...
  void SaveConfig(Json* p_out) const override {
    auto& out = *p_out;
    out["name"] = String("survival:boxhed_gpu");
  }
  void LoadConfig(Json const&) override {}
};

// register the objective function
XGBOOST_REGISTER_OBJECTIVE(BoXHEDRegression_GPU, "survival:boxhed_gpu")
.describe("BoXHED for censored survival data GPU version.")
.set_body([]() { return new BoXHEDRegression_GPU(); });
