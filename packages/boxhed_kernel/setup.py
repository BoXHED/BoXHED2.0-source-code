...
USER_OPTIONS = {
    ...
    # Python specific
    'use-system-libboxhed_kernel': (None, 'Use libboxhed_kernel.so in system path.', 0)
}
...
class BuildExt(build_ext.build_ext):  # pylint: disable=too-many-ancestors
    ...
    def build_cmake_extension(self):
        ...
        if USER_OPTIONS['use-system-libboxhed_kernel'][2]:
            self.logger.info('Using system libboxhed_kernel.')
            return
...
class InstallLib(install_lib.install_lib):
    ...
    def install(self):
        ...
        if USER_OPTIONS['use-system-libboxhed_kernel'][2] != 0:
            self.logger.info('Using system libboxhed_kernel.')
            lib_path = os.path.join(sys.prefix, 'lib')
            msg = 'use-system-libboxhed_kernel is specified, but ' + lib_name() + \
