    ...
    def __enter__(self):
        ...
        if serializer_version < self.serializer_min_version:
            raise ValueError(
                ...
                f"but the current deserializer in boxhed_shap requires at least version {self.serializer_min_version}."
            )
        if serializer_version > self.serializer_max_version:
            raise ValueError(
                f"The file being loaded was saved with a serializer version of {serializer_version}, " + \
                f"but the current deserializer in boxhed_shap only support up to version {self.serializer_max_version}."
            )
        ...
        if block_version < self.block_min_version:
            raise ValueError(
                ...
                f"but the current deserializer in boxhed_shap requires at least version {self.block_min_version}."
            )
        if block_version > self.block_max_version:
            raise ValueError(
                f"The file being loaded was saved with a block version of {block_version}, " + \
                f"but the current deserializer in boxhed_shap only support up to version {self.block_max_version}."
            )
        return self
