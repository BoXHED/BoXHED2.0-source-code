...
def embedding(ind, shap_values, feature_names=None, method="pca", alpha=1.0, show=True):
    """ Use the boxhed_shap values as an embedding which we project to 2D for visualization.
    ...
    ind : int or string
        ...
        boxhed_shap value over all the samples), or "sum()" to mean the sum of all the boxhed_shap values,
    ...
    cb = pl.colorbar()
    cb.set_label("boxhed_shap value for\n"+fname, size=13)
