...
def text(shap_values, num_starting_labels=0, grouping_threshold=0.01, separator='', xmin=None, xmax=None, cmax=None, display=True):
    ...
    boxhed_shap value assigned to that token.

    Parameters
    ----------
    shap_values : [numpy.array]
        List of arrays of boxhed_shap values. Each array has the boxhed_shap values for a string(# input_tokens x output_tokens).

    num_starting_labels : int
        Number of tokens (sorted in decending order by corresponding boxhed_shap values) that are uncovered in the initial view. When set to 0 all tokens
        covered. 
    ...
    xmin : float
        Minimum boxhed_shap value bound. 

    xmax : float
        Maximum boxhed_shap value bound.

    cmax : float
        Maximum absolute boxhed_shap value for sample. Used for scaling colors for input tokens.
    ...
    if len(shap_values.shape) == 2 and (shap_values.output_names is None or isinstance(shap_values.output_names, str)):
        ...
        for i, v in enumerate(shap_values):
            values, clustering = unpack_boxhed_shap_explanation_contents(v)
    ...
    if len(shap_values.shape) == 2 and shap_values.output_names is not None:
        ...
        for i in range(shap_values.shape[-1]):
            values, clustering = unpack_boxhed_shap_explanation_contents(shap_values[:,i])
    ...
    if len(shap_values.shape) == 3:
        ...
        for i in range(shap_values.shape[-1]):
            for j in range(shap_values.shape[0]):
                values, clustering = unpack_boxhed_shap_explanation_contents(shap_values[j,:,i])
...
<div align="center" style="margin-top: -35px;"><div style="display: inline-block; background: #fff; padding: 5px; color: #999; font-family: monospace">[{i}]</div>
    ...
    values, clustering = unpack_boxhed_shap_explanation_contents(shap_values)
...
def saliency_plot(shap_values):
    ...
    unpacked_values, clustering = unpack_boxhed_shap_explanation_contents(shap_values)
    ...
    def compress_boxhed_shap_matrix(boxhed_shap_matrix,group_sizes):
        compressed_matrix = np.zeros((group_sizes.shape[0],boxhed_shap_matrix.shape[1]))
        counter = 0
        for index in range(len(group_sizes)):
            compressed_matrix[index,:] = np.sum(boxhed_shap_matrix[counter:counter+group_sizes[index],:],axis=0)
            counter+=group_sizes[index]

        return compressed_matrix

    compressed_boxhed_shap_matrix = compress_boxhed_shap_matrix(shap_values.values,group_sizes)

    # generate background colors of saliency plot

    def get_colors(shap_values):
        input_colors = []
        cmax = max(abs(compressed_boxhed_shap_matrix.min()), abs(compressed_boxhed_shap_matrix.max()))
        for row_index in range(compressed_boxhed_shap_matrix.shape[0]):
            input_colors_row = []
            for col_index in range(compressed_boxhed_shap_matrix.shape[1]):
                scaled_value = 0.5 + 0.5 * compressed_boxhed_shap_matrix[row_index,col_index] / cmax
                color = colors.red_transparent_blue(scaled_value)
    ...
    for j in range(compressed_boxhed_shap_matrix.shape[0]):
        out += '<th>' + tokens[j].replace("<", "&lt;").replace(">", "&gt;").replace(' ##', '').replace('▁', '').replace('Ġ','') + '</th>'
    out += '</tr>'

    for row_index in range(compressed_boxhed_shap_matrix.shape[1]):
        out += '<tr>'
        out += '<th>' + model_output[row_index].replace("<", "&lt;").replace(">", "&gt;").replace(' ##', '').replace('▁', '').replace('Ġ','') + '</th>'
        for col_index in range(compressed_boxhed_shap_matrix.shape[0]):
            out += '<th style="background:' + input_colors[col_index][row_index]+ '">' + str(round(compressed_boxhed_shap_matrix[col_index][row_index],3)) + '</th>'
        out += '</tr>'
...
def heatmap(shap_values):
    ...
    def get_color(boxhed_shap_value,cmax):
        scaled_value = 0.5 + 0.5 * boxhed_shap_value / cmax
        color = colors.red_transparent_blue(scaled_value)
        color = (color[0]*255, color[1]*255, color[2]*255, color[3])
        return color
    ...
    # generate dictionary containing precomputed background colors and boxhed_shap values which are addressable by html token ids
    colors_dict = {}
    shap_values_dict = {}
    ...
    for col_index in range(len(model_output)):
        ...
        for row_index in range(processed_values[col_index]['collapsed_node_ids'].shape[0]):
            ...
            boxhed_shap_label_value_str = str(round(processed_values[col_index]['values'][row_index],3))
            if processed_values[col_index]['group_sizes'][row_index] > 1:
                boxhed_shap_label_value_str += ('/' + str(processed_values[col_index]['group_sizes'][row_index]))

            shap_values_list[uuid+'_input_node_'+str(processed_values[col_index]['collapsed_node_ids'][row_index])+'_label'] = boxhed_shap_label_value_str
        ...
        <script>
            ...
            function setLabelValues_{uuid}(id) {{
                for(const token in shap_values_{uuid}[id]){{
                    ...
                    document.getElementById(token).nextElementSibling.title = 'boxhed_shap Value : ' + shap_values_{uuid}[id][token];
                }}
            }}
...
def unpack_boxhed_shap_explanation_contents(shap_values):
