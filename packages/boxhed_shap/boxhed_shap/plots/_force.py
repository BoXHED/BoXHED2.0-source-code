""" Visualize the boxhed_shap values with additive force style layouts.
"""
...
def force(base_value, shap_values=None, features=None, feature_names=None, out_names=None, link="identity",
    ...
    """ Visualize the given boxhed_shap values with an additive force layout.

    Parameters
    ----------
    base_value : float
        This is the reference value that the feature contributions start from. For boxhed_shap values it should
        be the value of explainer.expected_value.

    shap_values : numpy.array
        Matrix of boxhed_shap values (# features) or (# samples x # features). If this is a 1D array then a single
    ...
    contribution_threshold : float
        ...
        Only features that the magnitude of their boxhed_shap value is larger than min_perc * (sum of all abs boxhed_shap values)
        will be displayed.
    """

    # support passing an explanation object
    if str(type(base_value)).endswith("Explanation'>"):
        boxhed_shap_exp = base_value
        base_value = boxhed_shap_exp.base_values
        shap_values = boxhed_shap_exp.values
        if features is None:
            if boxhed_shap_exp.display_data is None:
                features = boxhed_shap_exp.data
            else:
                features = boxhed_shap_exp.display_data
    ...
    if (isinstance(base_value, np.ndarray) or type(base_value) == list):
        if not isinstance(shap_values, list) or len(shap_values) != len(base_value):
            raise Exception("In v0.20 force_plot now requires the base value as the first parameter! " \
                            "Try boxhed_shap.force_plot(explainer.expected_value, shap_values) or " \
                            "for multi-output models try " \
                            "boxhed_shap.force_plot(explainer.expected_value[0], shap_values[0]).")
    ...
    if shap_values.shape[0] == 1:
        ...
        if shap_values.shape[0] > 3000:
            warnings.warn("boxhed_shap.plots.force is slow for many thousands of rows, try subsampling your data.")
...
def initjs():
    assert have_ipython, "IPython must be installed to use initjs()! Run `pip install ipython` and then restart boxhed_shap."
...
def save_html(out_file, plot, full_html=True):
    ...
    plot : BaseVisualizer
        Visualizer returned by boxhed_shap.force_plot()
    ...
    assert isinstance(plot, BaseVisualizer), "save_html requires a Visualizer returned by boxhed_shap.force_plot()."
    internal_open = False
...
class SimpleListVisualizer(BaseVisualizer):
    ...
    def html(self):
        # assert have_ipython, "IPython must be installed to use this visualizer! Run `pip install ipython` and then restart boxhed_shap."
        return """
<div id='{id}'>{err_msg}</div>
 <script>
   if (window.boxhed_shap) boxhed_shap.ReactDom.render(
    boxhed_shap.React.createElement(boxhed_shap.SimpleListVisualizer, {data}),
...
<div id='{id}'>{err_msg}</div>
 <script>
   if (window.boxhed_shap) boxhed_shap.ReactDom.render(
    boxhed_shap.React.createElement(boxhed_shap.AdditiveForceVisualizer, {data}),
    document.getElementById('{id}')
  );
</script>""".format(err_msg=err_msg, data=json.dumps(self.data), id=id_generator())
...
<div id='{id}'>{err_msg}</div>
 <script>
   if (window.boxhed_shap) boxhed_shap.ReactDom.render(
    boxhed_shap.React.createElement(boxhed_shap.AdditiveForceArrayVisualizer, {data}),
    document.getElementById('{id}')
  );
</script>""".format(err_msg=err_msg, data=json.dumps(self.data), id=id_generator())
