...
def waterfall(shap_values, max_display=10, show=True):
    ...
    The boxhed_shap value of a feature represents the impact of the evidence provided by that feature on the model's
    output. The waterfall plot is designed to visually display how the boxhed_shap values (evidence) of each feature
    move the model output from our prior expectation under the background data distribution, to the final model
    prediction given the evidence of all the features. Features are sorted by the magnitude of their boxhed_shap values
    ...
    shap_values : Explanation
        A one-dimensional Explanation object that contains the feature values and boxhed_shap values to plot.
    ...
    if (type(base_values) == np.ndarray and len(base_values) > 0) or type(base_values) == list:
        raise Exception("waterfall_plot requires a scalar base_values of the model output as the first "
                        ...
                        "Try boxhed_shap.waterfall_plot(explainer.base_values[0], values[0], X[0]) or "
                        "for multi-output models try "
                        "boxhed_shap.waterfall_plot(explainer.base_values[0], values[0][0], X[0]).")
...
def waterfall_legacy(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True):
    ...
    shap_values : numpy.array
        One dimensional array of boxhed_shap values.
    ...
    if str(type(expected_value)).endswith("Explanation'>"):
        boxhed_shap_exp = expected_value
        expected_value = boxhed_shap_exp.expected_value
        shap_values = boxhed_shap_exp.values
        features = boxhed_shap_exp.data
        feature_names = boxhed_shap_exp.feature_names
        lower_bounds = getattr(boxhed_shap_exp, "lower_bounds", None)
        upper_bounds = getattr(boxhed_shap_exp, "upper_bounds", None)
    ...
    if (type(expected_value) == np.ndarray and len(expected_value) > 0) or type(expected_value) == list:
        raise Exception("waterfall_plot requires a scalar expected_value of the model output as the first "
                        "parameter, but you have passed an array as the first parameter! "
                        "Try boxhed_shap.waterfall_plot(explainer.expected_value[0], shap_values[0], X[0]) or "
                        "for multi-output models try "
                        "boxhed_shap.waterfall_plot(explainer.expected_value[0], shap_values[0][0], X[0]).")
