""" Visualize cumulative boxhed_shap values."""
...
def __change_boxhed_shap_base_value(base_value, new_base_value, shap_values) -> np.ndarray:
    """Shift boxhed_shap base value to a new value. This function assumes that `base_value` and `new_base_value` are scalars
    ...
    # cube of boxhed_shap_interaction_values
...
def __decision_plot_matplotlib(
    ...
    # plot each observation's cumulative boxhed_shap values.
    ax = pl.gca()
    ax.set_xlim(xlim)
...
class DecisionPlotResult:
    ...
    def __init__(self, base_value, shap_values, feature_names, feature_idx, xlim):
        ...
        shap_values : numpy.ndarray
            The `shap_values` passed to decision_plot re-ordered based on `feature_order`. If boxhed_shap interaction values
            are passed to decision_plot, `shap_values` is a 2D (matrix) representation of the interactions. See
            `feature_names` to locate the feature positions. If `new_base_value` is specified, the boxhed_shap values are
...
def decision(
    ...
    """Visualize model decisions using cumulative boxhed_shap values.
    ...
    shap_values : numpy.ndarray
        Matrix of boxhed_shap values (# features) or (# samples x # features) from explainer.shap_values(). Or cube of boxhed_shap
        interaction values (# samples x # features x # features) from explainer.boxhed_shap_interaction_values().
    ...
    new_base_value : float
        boxhed_shap values are relative to a base value; by default, the expected value of the model's raw predictions. Use
    ...
    if isinstance(base_value, list) or isinstance(shap_values, list):
        raise TypeError("Looks like multi output. Try base_value[i] and shap_values[i], "
                        "or use boxhed_shap.multioutput_decision_plot().")
    ...
    if new_base_value is not None:
        shap_values = __change_boxhed_shap_base_value(base_value, new_base_value, shap_values)
    ...
    if not ignore_warnings:
        ...
        if feature_count * observation_count > 100000000:
            raise RuntimeError("Processing boxhed_shap values for {} features over {} observations may be slow. Set "
                               "ignore_warnings=True to ignore this "
...
def multioutput_decision(base_values, shap_values, row_index, **kwargs) -> Union[DecisionPlotResult, None]:
    ...
    unless new_base_value is specified. Supports both boxhed_shap values and boxhed_shap interaction values.
    ...
    shap_values : list of numpy.ndarray
        A multioutput list of boxhed_shap matrices or boxhed_shap cubes from explainer.shap_values() or
        explainer.boxhed_shap_interaction_values(), respectively.
    ...
    **kwargs : Any
    ...
    if shap_values.ndim not in [3, 4]:
        raise ValueError("The shap_values arg should be a list of two or three dimensional boxhed_shap arrays.")
    if shap_values.shape[0] != base_values.shape[0]:
        raise ValueError("The base_values output length is different than shap_values.")

    # shift boxhed_shap base values to mean of base values
    base_values_mean = base_values.mean()
    for i in range(shap_values.shape[0]):
        shap_values[i] = __change_boxhed_shap_base_value(base_values[i], base_values_mean, shap_values[i])
