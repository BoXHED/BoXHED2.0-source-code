""" Summary plots of boxhed_shap values across a whole dataset.
"""
...
def beeswarm(shap_values, max_display=10, order=Explanation.abs.mean(0),
    ...
    """Create a boxhed_shap beeswarm plot, colored by feature values when they are provided.

    Parameters
    ----------
    shap_values : Explanation
        This is an Explanation object containing a matrix of boxhed_shap values (# samples x # features).
    ...
    boxhed_shap_exp = shap_values
    # we make a copy here, because later there are places that might modify this array
    values = np.copy(boxhed_shap_exp.values)
    features = boxhed_shap_exp.data
    if sp.sparse.issparse(features):
        features = features.toarray()
    feature_names = boxhed_shap_exp.feature_names
    # if out_names is None: # TODO: waiting for slicer support
    #     out_names = boxhed_shap_exp.output_names
    ...
    if partition_tree is not None:
        assert partition_tree.shape[1] == 4, "The clustering provided by the Explanation object does not seem to be a partition tree (which is all boxhed_shap.plots.bar supports)!"

    # plotting boxhed_shap interaction values
    ...
    for pos, i in enumerate(reversed(feature_inds)):
        ...
        boxhed_shaps = values[:, i]
        fvalues = None if features is None else features[:, i]
        inds = np.arange(len(boxhed_shaps))
        np.random.shuffle(inds)
        if fvalues is not None:
            fvalues = fvalues[inds]
        boxhed_shaps = boxhed_shaps[inds]
        ...
        try:
            ...
            colored_feature = False
        N = len(boxhed_shaps)
        # hspacing = (np.max(boxhed_shaps) - np.min(boxhed_shaps)) / 200
        # curr_bin = []
        nbins = 100
        quant = np.round(nbins * (boxhed_shaps - np.min(boxhed_shaps)) / (np.max(boxhed_shaps) - np.min(boxhed_shaps) + 1e-8))
        ...
        if safe_isinstance(color, "matplotlib.colors.Colormap") and features is not None and colored_feature:
            ...
            assert features.shape[0] == len(boxhed_shaps), "Feature and boxhed_shap matrices must have the same number of rows!"

            # plot the nan fvalues in the interaction feature as grey
            nan_mask = np.isnan(fvalues)
            pl.scatter(boxhed_shaps[nan_mask], pos + ys[nan_mask], color="#777777", vmin=vmin,
                        vmax=vmax, s=16, alpha=alpha, linewidth=0,
                        zorder=3, rasterized=len(boxhed_shaps) > 500)
            ...
            pl.scatter(boxhed_shaps[np.invert(nan_mask)], pos + ys[np.invert(nan_mask)],
                        cmap=color, vmin=vmin, vmax=vmax, s=16,
                        c=cvals, alpha=alpha, linewidth=0,
                        zorder=3, rasterized=len(boxhed_shaps) > 500)
        else:

            pl.scatter(boxhed_shaps, pos + ys, s=16, alpha=alpha, linewidth=0, zorder=3,
                        color=color if colored_feature else "#777777", rasterized=len(boxhed_shaps) > 500)
...
def summary_legacy(shap_values, features=None, feature_names=None, max_display=None, plot_type=None,
    ...
    shap_values : numpy.array
        For single output explanations this is a matrix of boxhed_shap values (# samples x # features).
        For multi-output explanations this is a list of such matrices of boxhed_shap values.
    ...
    if str(type(shap_values)).endswith("Explanation'>"):
        boxhed_shap_exp = shap_values
        base_value = boxhed_shap_exp.base_values
        shap_values = boxhed_shap_exp.values
        if features is None:
            features = boxhed_shap_exp.data
        if feature_names is None:
            feature_names = boxhed_shap_exp.feature_names
    ...
    if plot_type == "dot":
        ...
        if features is not None:
            ...
            for pos, i in enumerate(feature_order):
                boxhed_shaps = shap_values[:, i]
                boxhed_shap_min, boxhed_shap_max = np.min(boxhed_shaps), np.max(boxhed_shaps)
                rng = boxhed_shap_max - boxhed_shap_min
                xs = np.linspace(np.min(boxhed_shaps) - rng * 0.2, np.max(boxhed_shaps) + rng * 0.2, 100)
                if np.std(boxhed_shaps) < (global_high - global_low) / 100:
                    ds = gaussian_kde(boxhed_shaps + np.random.randn(len(boxhed_shaps)) * (global_high - global_low) / 100)(xs)
                else:
                    ds = gaussian_kde(boxhed_shaps)(xs)
                ds /= np.max(ds) * 3

                values = features[:, i]
                window_size = max(10, len(values) // 20)
                smooth_values = np.zeros(len(xs) - 1)
                sort_inds = np.argsort(boxhed_shaps)
                trailing_pos = 0
                leading_pos = 0
                running_sum = 0
                back_fill = 0
                for j in range(len(xs) - 1):
                    ...
                    while leading_pos < len(boxhed_shaps) and xs[j] >= boxhed_shaps[sort_inds[leading_pos]]:
                ...
                # plot the nan values in the interaction feature as grey
                nan_mask = np.isnan(values)
                pl.scatter(boxhed_shaps[nan_mask], np.ones(shap_values[nan_mask].shape[0]) * pos,
                ...
                cvals[cvals_imp > vmax] = vmax
                cvals[cvals_imp < vmin] = vmin
                pl.scatter(boxhed_shaps[np.invert(nan_mask)], np.ones(shap_values[np.invert(nan_mask)].shape[0]) * pos,
        ...
        boxhed_shap_min, boxhed_shap_max = np.min(shap_values), np.max(shap_values)
        x_points = np.linspace(boxhed_shap_min, boxhed_shap_max, num_x_points)
        ...
        for pos, ind in enumerate(feature_order):
            ...
            for i in range(nbins):
                # get boxhed_shap values in this bin:
                boxhed_shaps = shap_values[order[thesebins[i]:thesebins[i + 1]], ind]
                # if there's only one element, then we can't
                if boxhed_shaps.shape[0] == 1:
                    warnings.warn(
                ...
                ys[i, :] = gaussian_kde(boxhed_shaps + np.random.normal(loc=0, scale=0.001, size=boxhed_shaps.shape[0]))(x_points)
        ...
        pl.xlim(boxhed_shap_min, boxhed_shap_max)
