...
def scatter(shap_values, color="#1E88E5", hist=True, axis_color="#333333", cmap=colors.red_blue,
            ...
            overlay=None, ax=None, ylabel="boxhed_shap value", show=True):
    """ Create a boxhed_shap dependence scatter plot, colored by an interaction feature.

    Plots the value of the feature on the x-axis and the boxhed_shap value of the same feature
    ...
    Parameters
    ----------
    shap_values : boxhed_shap.Explanation
        A single column of a boxhed_shap Explanation object (i.e. shap_values[:,"Feature A"]).

    color : string or boxhed_shap.Explanation
        ...
        This is calculated using boxhed_shap.utils.approximate_interactions.
    ...
    assert str(type(shap_values)).endswith("Explanation'>"), "The shap_values paramemter must be a boxhed_shap.Explanation object!"
    ...
    if len(shap_values.shape) != 1:
        raise Exception("The passed Explanation object has multiple columns, please pass a single feature column to " + \
                        "boxhed_shap.plots.dependence like: shap_values[:,column]")
    ...
    # TODO: This stacking could be avoided if we use the new boxhed_shap.utils.potential_interactions function
...
def dependence_legacy(ind, shap_values=None, features=None, feature_names=None, display_features=None,
                      ...
                      ymin=None, ymax=None):
    """ Create a boxhed_shap dependence plot, colored by an interaction feature.
    ...
    ind : int or string
        ...
        the feature with that rank (ordered by mean absolute boxhed_shap value over all the samples).

    shap_values : numpy.array
        Matrix of boxhed_shap values (# samples x # features).
    ...
    interaction_index : "auto", None, int, or string
        ...
        as a string. If "auto" then boxhed_shap.common.approximate_interactions is used to pick what
        seems to be the strongest interaction (note that to find to true stongest interaction you
        need to compute the boxhed_shap interaction values).
