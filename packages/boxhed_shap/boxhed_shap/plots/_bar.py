...
def bar(shap_values, max_display=10, order=Explanation.abs, clustering=None, clustering_cutoff=0.5,
    ...
    """ Create a bar plot of a set of boxhed_shap values.

    If a single sample is passed then we plot the boxhed_shap values as a bar chart. If an
    ...
    Parameters
    ----------
    shap_values : boxhed_shap.Explanation or boxhed_shap.Cohorts or dictionary of boxhed_shap.Explanation objects
        A single row of a boxhed_shap Explanation object (i.e. shap_values[0]) or a multi-row Explanation
...
def bar_legacy(shap_values, features=None, feature_names=None, max_display=None, show=True):
    ...
    pl.xlabel("boxhed_shap value (impact on model output)")
