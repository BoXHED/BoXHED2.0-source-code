...
def partial_dependence(ind, model, data, xmin="percentile(0)", xmax="percentile(100)",
    ...
    if type(ind) is not tuple:
        ...
        if shap_values is not None:
            ...
            # if boxhed_shap_value_features is None:
            #     boxhed_shap_value_features = features
            ...
            #     else:
            #         model_expected_value = model(features).mean()
            # if str(type(boxhed_shap_value_features)).endswith("'pandas.core.frame.DataFrame'>"):
            #     boxhed_shap_value_features = boxhed_shap_value_features.values
