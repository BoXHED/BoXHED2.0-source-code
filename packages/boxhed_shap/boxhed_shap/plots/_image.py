...
from boxhed_shap._explanation import Explanation
...
def image(shap_values: Explanation or np.ndarray,
    ...
    """ Plots boxhed_shap values for image inputs.

    Parameters
    ----------
    shap_values : [numpy.array]
        List of arrays of boxhed_shap values. Each array has the boxhed_shap (# samples x width x height x channels), and the
    ...
    if str(type(shap_values)).endswith("Explanation'>"):
        ...
        # ind = 0
        if len(boxhed_shap_exp.output_dims) == 1:
            shap_values = [boxhed_shap_exp.values[..., i] for i in range(boxhed_shap_exp.values.shape[-1])]
        elif len(boxhed_shap_exp.output_dims) == 0:
        ...
        if pixel_values is None:
            pixel_values = boxhed_shap_exp.data
        if labels is None:
            labels = boxhed_shap_exp.output_names
    ...
    cb = fig.colorbar(im, ax=np.ravel(axes).tolist(), label="boxhed_shap value", orientation="horizontal",
...
def image_to_text(shap_values):
    """ Plots boxhed_shap values for image inputs with test outputs.

    Parameters
    ----------
    shap_values : [numpy.array]
        List of arrays of boxhed_shap values. Each array has the boxhed_shap (# width x height x channels x num output tokens). One array
        for each sample
    ...
    # computing boxhed_shap color values for every pixel and for every output token
            ...
            <div id="{uuid}_image_viz_input_container" style="padding:15px;border-style:solid;margin:5px;flex:2;">
              ...
              <div id="{uuid}_image_viz_input_content" style="margin:5px;font-family:sans-serif;">
                  ...
                  <div id="{uuid}_tools">
                      ...
                      <span style="font-size:12px;margin-right:15px;"> boxhed_shap-Overlay Opacity </span>
        ...
        <script>
            ...
            function {uuid}_update_image_and_overlay(selected_id) {{
                if (selected_id == null) {{
                    ...
                    {uuid}_currImagData = {uuid}_blend_image_boxhed_shap_map({uuid}_image_data_gray_scale, {uuid}_shap_values_color_dict[selected_id], {image_height}, {image_width}, {uuid}_opacity, {uuid}_context);
                    {uuid}_redraw();
                }}
            }}
            ...
            function {uuid}_set_opacity(value) {{
                ...
                if ({uuid}_heatmap_flat_state !== null ) {{
                    {uuid}_currImagData = {uuid}_blend_image_boxhed_shap_map({uuid}_image_data_gray_scale, {uuid}_shap_values_color_dict[{uuid}_heatmap_flat_state], {image_height}, {image_width}, {uuid}_opacity, {uuid}_context);                    
                    {uuid}_redraw();
                }}
            }}
            ...
            function {uuid}_blend_image_boxhed_shap_map(image_data_matrix, boxhed_shap_color_map, image_height, image_width, alpha, context) {{
                ...
                for(var row_index = 0; row_index < image_height; row_index++) {{
                    ...
                    for(var col_index = 0; col_index < image_width; col_index++) {{
                        ...
                        blendedImageData.data[index + 0] = image_data_matrix[row_index][col_index][0] * alpha + (boxhed_shap_color_map[row_index][col_index][0]) * ( 1 - alpha);
                        blendedImageData.data[index + 1] = image_data_matrix[row_index][col_index][1] * alpha + (boxhed_shap_color_map[row_index][col_index][1]) * ( 1 - alpha);
                        blendedImageData.data[index + 2] = image_data_matrix[row_index][col_index][2] * alpha + (boxhed_shap_color_map[row_index][col_index][2]) * ( 1 - alpha);
                        blendedImageData.data[index + 3] = image_data_matrix[row_index][col_index][3] * alpha + (boxhed_shap_color_map[row_index][col_index][3]) * ( 1 - alpha);
                    }}
                }}
