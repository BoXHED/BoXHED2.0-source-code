...
class MimicExplainer(Explainer):
    ...
    Tree boxhed_shap allows for very fast boxhed_shap value explainations of flexible gradient boosted decision
    tree (GBDT) models. Since GBDT models are so flexible we can train them to mimic any black-box
    model and then using Tree boxhed_shap we can explain them. This won't work well for images, but for
    ...
    def __init__(self, model, data, mimic_model="xgboost", mimic_model_params={}):
        ...
        assert isinstance(self.data, DenseData), "boxhed_shap explainer only supports the DenseData input currently."
        assert not self.data.transposed, "boxhed_shap explainer does not support transposed DenseData currently."
        ...
        if len(self.data.weights) < 100:
            log.warning("Using only " + str(len(self.data.weights)) + " training data samples could cause " +
                        ...
                        "or if you don't have more samples, using boxhed_shap.inflate(data, N) to generate more.")
    ...
    def shap_values(self, X, **kwargs):
        ...
        -------
        array or list
            For a models with a single output this returns a matrix of boxhed_shap values
