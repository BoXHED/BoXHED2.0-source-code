...
class Explainer(Serializable):
    """ Uses boxhed_shapley values to explain any machine learning model or python function.

    This is the primary explainer interface for the boxhed_shap library. It takes any combination
    ...
    def __init__(self, model, masker=None, link=links.identity, algorithm="auto", output_names=None, feature_names=None, linearize_link=True,
        ...
        masker : function, numpy.array, pandas.DataFrame, tokenizer, None, or a list of these for each model input
            ...
            As a shortcut for the standard masking using by boxhed_shap you can pass a background data matrix
            instead of a function and that matrix will be used for masking. Domain specific masking
            functions are available in boxhed_shap such as boxhed_shap.ImageMasker for images and boxhed_shap.TokenMasker
            ...
            boxhed_shap.TabularMasker(data, hclustering="correlation") will enforce a hierarchial clustering
        ...
        algorithm : "auto", "permutation", "partition", "tree", or "linear"
            The algorithm used to estimate the boxhed_shapley values. There are many different algorithms that
            can be used to estimate the boxhed_shapley values (and the related value for constrained games), each
        ...
        # wrap the incoming masker object as a boxhed_shap.Masker object
        ...
        elif safe_isinstance(self.model, "boxhed_shap.models.TeacherForcing") and safe_isinstance(self.masker, ["boxhed_shap.maskers.Text", "boxhed_shap.maskers.Image"]):
            self.masker = maskers.OutputComposite(self.masker, self.model.text_generate)
        elif safe_isinstance(self.model, "boxhed_shap.models.TopKLM") and safe_isinstance(self.masker, "boxhed_shap.maskers.Text"):
    ...
    def save(self, out_file, model_saver=".save", masker_saver=".save"):
        ...
        """
        super().save(out_file)
        with Serializer(out_file, "boxhed_shap.Explainer", version=0) as s:
    ...
    def load(cls, in_file, model_loader=Model.load, masker_loader=Masker.load, instantiate=True):
        ...
        with Deserializer(in_file, "boxhed_shap.Explainer", min_version=0, max_version=0) as s:
