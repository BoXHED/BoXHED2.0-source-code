"""
This module is a pure python implementation of Tree boxhed_shap.
...
#         # shortcut using the C++ version of Tree boxhed_shap in XGBoost and LightGBM
...
#             for t in self.trees:
#                 self.tree_boxhed_shap(t, X, x_missing, phi)
#             phi /= len(self.trees)
...
#                 for t in self.trees:
#                     self.tree_boxhed_shap(t, X[i,:], x_missing, phi[i,:,:])
#             phi /= len(self.trees)
...
#     def boxhed_shap_interaction_values(self, X, tree_limit=-1, **kwargs):
...
#     def tree_boxhed_shap(self, tree, x, x_missing, phi, condition=0, condition_feature=0):

#         # start the recursive algorithm
#         boxhed_shap._cext.tree_boxhed_shap(
...
#         self.max_depth = boxhed_shap._cext.compute_expectations(
...
class TreeExplainer:
    """ A pure Python (slow) implementation of Tree boxhed_shap.
    """
...
class Tree:
    ...
    def __init__(self, tree, normalize=False):
        if str(type(tree)).endswith("'sklearn.tree._tree.Tree'>"):
            ...
            # we recompute the expectations to make sure they follow the boxhed_shap logic
            self.max_depth = compute_expectations(
                self.children_left, self.children_right, self.node_sample_weight,
                self.values, 0
            )
...
#@numba.jit(nopython=True, nogil=True)
def tree_boxhed_shap_recursive(children_left, children_right, children_default, features, thresholds, values, node_sample_weight,
