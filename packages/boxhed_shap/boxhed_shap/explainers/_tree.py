...
class Tree(Explainer):
    """ Uses Tree boxhed_shap algorithms to explain the output of ensemble tree models.

    Tree boxhed_shap is a fast and exact method to estimate boxhed_shap values for tree models and ensembles of trees,
    ...
    def __init__(self, model, data = None, model_output="raw", feature_perturbation="interventional", feature_names=None, approximate=False, **deprecated_options):
        ...
        feature_perturbation : "interventional" (default) or "tree_path_dependent" (default when data=None)
            Since boxhed_shap values rely on conditional expectations we need to decide how to handle correlated
        ...
        model_output : "raw", "probability", "log_loss", or model method name
            ...
            (note that this means the boxhed_shap values now sum to the probability output of the model). If "logloss"
            then we explain the log base e of the model loss function, so that the boxhed_shap values sum up to the
        ...
        Examples
        --------
        See `Tree explainer examples <https://boxhed_shap.readthedocs.io/en/latest/api_examples/explainers/Tree.html>`_
        """
        ...
        if getattr(self.masker, "clustering", None) is not None:
            raise ExplainerError("TreeExplainer does not support clustered data inputs! Please use boxhed_shap.Explainer or pass an unclustered masker!")
        ...
        if self.data is None:
            ...
            #warnings.warn("Setting feature_perturbation = \"tree_path_dependent\" because no background data was given.")
                ...
                warnings.warn("Passing "+str(self.data.shape[0]) + " background samples may lead to slow runtimes. Consider "
                    "using boxhed_shap.sample(data, 100) to create a smaller background data set.")
        ...
        if safe_isinstance(model, "xgboost.sklearn.XGBClassifier") and self.model.model_output != "raw":
            import boxhed_kernel as xgboost
        ...
        if self.model.model_output == "log_loss":
            ...
            try:
                ...
                raise ExplainerError("Currently TreeExplainer can only handle models with categorical splits when " \
                                ...
                                "boxhed_shap.TreeExplainer(model, feature_perturbation=\"tree_path_dependent\").")
    ...
    def __call__(self, X, y=None, interactions=False, check_additivity=True):
        ...
        if not interactions:
            ...
            v = self.boxhed_shap_interaction_values(X)
    ...
    def shap_values(self, X, y=None, tree_limit=None, approximate=False, check_additivity=True, from_call=False):
        """ Estimate the boxhed_shap values for a set of samples.
        ...
        approximate : bool
            Run fast, but only roughly approximate the Tree boxhed_shap values. This runs a method
            previously proposed by Saabas which only considers a single feature ordering. Take care
            since this does not have the consistency guarantees of boxhed_shapley values and places too
            much weight on lower splits in the tree.

        check_additivity : bool
            Run a validation check that the sum of the boxhed_shap values equals the output of the model. This
        ...
        -------
        array or list
            For models with a single output this returns a matrix of boxhed_shap values
        ...
        # shortcut using the C++ version of Tree boxhed_shap in XGBoost, LightGBM, and CatBoost
            ...
            if self.model.model_type == "xgboost":
                ...
                try:
                    phi = self.model.original_model.predict(
                        ...
                        raise ValueError("This reshape error is often caused by passing a bad data matrix to boxhed_shap. " \
                                         "See https://github.com/slundberg/boxhed_shap/issues/580") from e
                ...
                if self.model.original_model.params['objective'] == 'binary':
                    if not from_call:
                        warnings.warn('LightGBM binary classifier with TreeExplainer boxhed_shap values output has changed to a list of ndarray')
                    phi = np.concatenate((0-phi, phi), axis=-1)
                if phi.shape[1] != X.shape[1] + 1:
                    try:
                        phi = phi.reshape(X.shape[0], phi.shape[1]//(X.shape[1]+1), X.shape[1]+1)
                    except ValueError as e:
                        raise Exception("This reshape error is often caused by passing a bad data matrix to boxhed_shap. " \
                                         "See https://github.com/slundberg/boxhed_shap/issues/580") from e
                ...
                phi = self.model.original_model.get_feature_importance(data=X, fstr_type='boxhed_shapValues')
        ...
        out = self._get_boxhed_shap_output(phi, flat_output)
        if check_additivity and self.model.model_output == "raw":
    ...
    def _get_boxhed_shap_output(self, phi, flat_output):
    ...
    def boxhed_shap_interaction_values(self, X, y=None, tree_limit=None):
        """ Estimate the boxhed_shap interaction values for a set of samples.
        ...
        -------
        array or list
            For models with a single output this returns a tensor of boxhed_shap values
            ...
            boxhed_shap value for that feature for that sample. The diagonal entries of the matrix represent the
        ...
        assert self.model.model_output == "raw", "Only model_output = \"raw\" is supported for boxhed_shap interaction values right now!"
        #assert self.feature_perturbation == "tree_path_dependent", "Only feature_perturbation = \"tree_path_dependent\" is supported for boxhed_shap interaction values right now!"
        transform = "identity"
        ...
        # shortcut using the C++ version of Tree boxhed_shap in XGBoost
        ...
        return self._get_boxhed_shap_interactions_output(phi,flat_output)

    # we pull off the last column and keep it as our expected_value
    def _get_boxhed_shap_interactions_output(self, phi, flat_output):
        if self.model.num_outputs == 1:
    ...
    def assert_additivity(self, phi, model_output):
        ...
        def check_sum(sum_val, model_output):
            ...
            if np.max(diff / (np.abs(sum_val) + 1e-2)) > 1e-2:
                ...
                err_msg += " This check failed because for one of the samples the sum of the boxhed_shap values" \
...
class TreeEnsemble:
    ...
    def __init__(self, model, data=None, data_missing=None, model_output=None):
        ...
        if type(model) is dict and "trees" in model:
        ...
        elif safe_isinstance(model, "xgboost.core.Booster") or safe_isinstance(model, "boxhed_kernel.core.Booster"):
            import boxhed_kernel as xgboost
            ...
            boxhed_shap_trees = [trees[param_idx] for trees in model.base_models]
            self.internal_dtype = boxhed_shap_trees[0].tree_.value.dtype.type
            self.input_dtype = np.float32
            scaling = - model.learning_rate * np.array(model.scalings) # output is weighted average of trees
            self.trees = [SingleTree(e.tree_, scaling=s, data=data, data_missing=data_missing) for e,s in zip(boxhed_shap_trees,scaling)]
            self.objective = objective_name_map.get(boxhed_shap_trees[0].criterion, None)
            self.tree_output = "raw_value"
...
class SingleTree:
    ...
    def __init__(self, tree, normalize=False, scaling=1.0, data=None, data_missing=None):
        ...
        # we compute the expectations to make sure they follow the boxhed_shap logic
        ...
        for i in range(self.num_trees):
            for j in range(self.num_nodes[i]):
                ...
                if self.node_cleft[i][j] >= 0:
                    # Xgboost uses < for thresholds where boxhed_shap uses <=
