...
from ..utils import MaskedModel, boxhed_shapley_coefficients, make_masks, delta_minimization_order
from ._explainer import Explainer

log = logging.getLogger('boxhed_shap')


class Exact(Explainer):
    """ Computes boxhed_shap values via an optimized exact enumeration.

    This works well for standard boxhed_shapley value maskers for models with less than ~15 features that vary
    ...
    minimize sequential differences. This is done using gray codes for standard boxhed_shapley values
    ...
    def __init__(self, model, masker, link=links.identity, linearize_link=True, feature_names=None):
        ...
        masker : function or numpy.array or pandas.DataFrame
            ...
            As a shortcut for the standard masking used by boxhed_shap you can pass a background data matrix
            instead of a function and that matrix will be used for masking. To use a clustering
            game structure you can pass a boxhed_shap.maskers.TabularPartitions(data) object.

        link : function
            The link function used to map between the output units of the model and the boxhed_shap value units. By
            default it is boxhed_shap.links.identity, but boxhed_shap.links.logit can be useful so that expectations are
    ...
    def explain_row(self, *row_args, max_evals, main_effects, error_bounds, batch_size, outputs, interactions, silent):
        ...
        # do the standard boxhed_shapley values
        inds = None
        if getattr(self.masker, "clustering", None) is None:
            ...
            if interactions is False or interactions is 1: # pylint: disable=literal-comparison
                ...
                coeff = boxhed_shapley_coefficients(len(inds))
            ...
            # boxhed_shapley-Taylor interaction values
        ...
        # do a partition tree constrained version of boxhed_shapley values
        else:
...
@jit
def _compute_grey_code_row_values(row_values, mask, inds, outputs, boxhed_shapley_coeff, extended_delta_indexes, noop_code):
    set_size = 0
    M = len(inds)
    for i in range(2**M):
        ...
        on_coeff = boxhed_shapley_coeff[set_size-1]
        if set_size < M:
            off_coeff = boxhed_shapley_coeff[set_size]
        out = outputs[i]
        for j in inds:
            if mask[j]:
...
@jit
def _compute_grey_code_row_values_st(row_values, mask, inds, outputs, boxhed_shapley_coeff, extended_delta_indexes, noop_code):
    set_size = 0
    M = len(inds)
    for i in range(2**M):
        ...
        for j in range(M):
            for k in range(j+1, M):
                if not mask[j] and not mask[k]:
                    delta = out * boxhed_shapley_coeff[set_size] # * 2
                elif (not mask[j] and mask[k]) or (mask[j] and not mask[k]):
                    delta = -out * boxhed_shapley_coeff[set_size - 1] # * 2
                else: # both true
                    delta = out * boxhed_shapley_coeff[set_size - 2] # * 2
