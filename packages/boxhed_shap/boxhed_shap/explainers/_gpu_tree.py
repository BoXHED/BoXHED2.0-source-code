...
class GPUTree(Tree):
    ...
    model_output : "raw", "probability", "log_loss", or model method name
        ...
        probability space (note that this means the boxhed_shap values now sum to the probability output of the
        model). If "logloss" then we explain the log base e of the model loss function, so that the boxhed_shap
    ...
    Examples
    --------
    See `GPUTree explainer examples <https://boxhed_shap.readthedocs.io/en/latest/api_examples/explainers/GPUTree.html>`_
    """
    ...
    def shap_values(self, X, y=None, tree_limit=None, approximate=False, check_additivity=True,
        ...
        check_additivity : bool
            Run a validation check that the sum of the boxhed_shap values equals the output of the
            model. This
        ...
        _cext_gpu.dense_tree_boxhed_shap(
    ...
    def boxhed_shap_interaction_values(self, X, y=None, tree_limit=None):
        ...
        array or list
            ...
            matrix sums to the
            boxhed_shap value for that feature for that sample. The diagonal entries of the matrix
            represent the
        ...
        assert self.model.model_output == "raw", "Only model_output = \"raw\" is supported for " \
                                                 "boxhed_shap interaction values right now!"
