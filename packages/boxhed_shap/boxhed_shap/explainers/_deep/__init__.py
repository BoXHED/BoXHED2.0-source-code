...
class Deep(Explainer):
    """ Meant to approximate boxhed_shap values for deep learning models.

    This is an enhanced version of the DeepLIFT algorithm (Deep boxhed_shap) where, similar to Kernel boxhed_shap, we
    approximate the conditional expectations of boxhed_shap values using a selection of background samples.
    Lundberg and Lee, NIPS 2017 showed that the per node attribution rules in DeepLIFT (Shrikumar,
    Greenside, and Kundaje, arXiv 2017) can be chosen to approximate boxhed_shapley values. By integrating
    over many backgound samples Deep estimates approximate boxhed_shap values such that they sum
    ...
    def __init__(self, model, data, session=None, learning_phase_flags=None):
        ...
        model : if framework == 'tensorflow', (input : [tf.Tensor], output : tf.Tensor)
            ...
            output of the model to be explained. Note that boxhed_shap values are specific to a single
            ...
            If the input is a tuple, the returned boxhed_shap values will be for the input of the
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order='max', check_additivity=True):
        """ Return approximate boxhed_shap values for the model applied to the data given by X.
        ...
        -------
        array or list
            For a models with a single output this returns a tensor of boxhed_shap values with the same shape
            as X. For a model with multiple outputs this returns a list of boxhed_shap value tensors, each of
