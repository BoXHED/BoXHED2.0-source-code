...
def custom_record_gradient(op_name, inputs, attrs, results):
    ...
    if op_name == "ResourceGather" and inputs[1].dtype == tf.int32:
        ...
        reset_input = True
    try:
        out = tf_backprop._record_gradient("boxhed_shap_"+op_name, inputs, attrs, results)
    except AttributeError:
        out = tf_backprop.record_gradient("boxhed_shap_"+op_name, inputs, attrs, results)
...
class TFDeep(Explainer):
    ...
    def __init__(self, model, data, session=None, learning_phase_flags=None):
        ...
        model : tf.keras.Model or (input : [tf.Operation], output : tf.Operation)
            ...
            specifies the input and output of the model to be explained. Note that boxhed_shap values
        ...
        # make a blank array that will get lazily filled in with the boxhed_shap value computation
    ...
    def phi_symbolic(self, i):
        """ Get the boxhed_shap value computation graph for a given model output.
        """
        if self.phi_symbolics[i] is None:
            ...
            if not tf.executing_eagerly():
                ...
                @tf.function
                def grad_graph(boxhed_shap_rAnD):
                    phase = tf.keras.backend.learning_phase()
                    tf.keras.backend.set_learning_phase(0)

                    with tf.GradientTape(watch_accessed_variables=False) as tape:
                        tape.watch(boxhed_shap_rAnD)
                        out = self.model(boxhed_shap_rAnD)
                        if self.multi_output:
                            out = out[:,i]

                    self._init_between_tensors(out.op, boxhed_shap_rAnD)
                    x_grad = tape.gradient(out, boxhed_shap_rAnD)
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", check_additivity=True):
        ...
        # check that the boxhed_shap values sum up to the model output
        if check_additivity:
            if not tf.executing_eagerly():
            ...
            for l in range(len(self.expected_value)):
                ...
                assert np.abs(diffs).max() < 1e-2, "The boxhed_shap explanations do not sum up to the model's output! This is either because of a " \
    ...
    def custom_grad(self, op, *grads):
        ...
        """
        type_name = op.type[5:] if op.type.startswith("boxhed_shap_") else op.type
        out = op_handlers[type_name](self, op, *grads) # we cut off the boxhed_shap_ prefex before the lookup
        return out
...
def maxpool(explainer, op, *grads):
    ...
    if op.type.startswith("boxhed_shap_"):
        op.type = op.type[5:]
