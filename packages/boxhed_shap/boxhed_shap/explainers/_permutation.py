...
class Permutation(Explainer):
    """ This method approximates the boxhed_shapley values by iterating through permutations of the inputs.
    ...
    If we do this once, then we get the exact boxhed_shap values for models with up to second order interaction effects.
    We can iterate this many times over many random permutations to get better boxhed_shap value estimates for models
    ...
    def __init__(self, model, masker, link=links.identity, feature_names=None, linearize_link=True, seed=None, **call_args):
        ...
        masker : function or numpy.array or pandas.DataFrame
            ...
            game structure you can pass a boxhed_shap.maksers.Tabular(data, clustering=\"correlation\") object.
        ...
        **call_args : valid argument to the __call__ method
        ...
        # setting seed for random generation: if seed is not None, then boxhed_shap values computation should be reproducible
        np.random.seed(seed)
    ...
    def explain_row(self, *row_args, max_evals, main_effects, error_bounds, batch_size, outputs, silent):
        ...
        if len(inds) > 0:
            for _ in range(npermutations):
                ...
                # update our boxhed_shap value estimates
                i = 0
    ...
    def shap_values(self, X, npermutations=10, main_effects=False, error_bounds=False, batch_evals=True, silent=False):
        """ Legacy interface to estimate the boxhed_shap values for a set of samples.
    ...
    def __str__(self):
        return "boxhed_shap.explainers.Permutation()"
