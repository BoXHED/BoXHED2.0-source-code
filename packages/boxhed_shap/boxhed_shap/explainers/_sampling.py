...
class Sampling(Kernel):
    """ This is an extension of the boxhed_shapley sampling values explanation method (aka. IME)

    SamplingExplainer computes boxhed_shap values under the assumption of feature independence and is an
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        # if more than one feature varies then we have to do real work
            ...
            # correct the sum of the boxhed_shap values to equal the output of the model using a linear
            # regression model with priors of the coefficents equal to the estimated variances for each
            # boxhed_shap value (note that 1e6 is designed to increase the weight of the sample and so closely
