...
class Linear(Explainer):
    """ Computes boxhed_shap values for a linear model, optionally accounting for inter-feature correlations.

    This computes the boxhed_shap values for a linear model and can account for the correlations among
    the input features. Assuming features are independent leads to interventional boxhed_shap values which
    ...
    feature_perturbation : "interventional" (default) or "correlation_dependent"
        There are two ways we might want to compute boxhed_shap values, either the full conditional boxhed_shap
        values or the interventional boxhed_shap values. For interventional boxhed_shap values we break any
        dependence structure between features in the model and so uncover how the model would behave if we
        intervened and changed some of the inputs. For the full conditional boxhed_shap values we respect
    ...
    Examples
    --------
    See `Linear explainer examples <https://boxhed_shap.readthedocs.io/en/latest/api_examples/explainers/Linear.html>`_
    """
    ...
    def __init__(self, model, masker, link=links.identity, nsamples=1000, feature_perturbation=None, **kwargs):
        ...
        # wrap the incoming masker object as a boxhed_shap.Masker object before calling
