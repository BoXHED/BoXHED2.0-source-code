...
class Gradient(Explainer):
    ...
    feature attribution method designed for differentiable models based on an extension of boxhed_shapley
    values to infinite player games (Aumann-boxhed_shapley values). Integrated gradients values are a bit
    different from boxhed_shap values, and require a single reference value to integrate from. As an adaptation
    to make them approximate boxhed_shap values, expected gradients reformulates the integral as an expectation
    ...
    def __init__(self, model, data, session=None, batch_size=50, local_smoothing=0):
        ...
        model : tf.keras.Model, (input : [tf.Tensor], output : tf.Tensor), torch.nn.Module, or a tuple
            ...
            is a tuple, the returned boxhed_shap values will be for the input of the layer argument. layer must
    ...
    def shap_values(self, X, nsamples=200, ranked_outputs=None, output_rank_order="max", rseed=None, return_variances=False):
        ...
        rseed : None or int
            Seeding the randomness in boxhed_shap value computation  (background example choice,
