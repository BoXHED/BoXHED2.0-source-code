...
class Kernel(Explainer):
    """Uses the Kernel boxhed_shap method to explain the output of any function.

    Kernel boxhed_shap is a method that uses a special weighted linear regression
    to compute the importance of each feature. The computed importance values
    are boxhed_shapley values from game theory and also coefficents from a local linear
    regression.
    ...
    data : numpy.array or pandas.DataFrame or boxhed_shap.common.DenseData or any scipy.sparse matrix
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        assert isinstance(self.data, DenseData) or isinstance(self.data, SparseData), \
               "boxhed_shap explainer only supports the DenseData and SparseData input currently."
        assert not self.data.transposed, "boxhed_shap explainer does not support transposed DenseData or SparseData currently."

        # warn users about large background data sets
        if len(self.data.weights) > 100:
            log.warning("Using " + str(len(self.data.weights)) + " background data samples could cause " +
                        "slower run times. Consider using boxhed_shap.sample(data, K) or boxhed_shap.kmeans(data, K) to " +
    ...
    def shap_values(self, X, **kwargs):
        ...
        nsamples : "auto" or int
            ...
            lead to lower variance estimates of the boxhed_shap values. The "auto" setting uses
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        # if more than one feature varies then we have to do real work
            ...
            # solve then expand the feature importance (boxhed_shapley value) vector to contain the non-varying features
            phi = np.zeros((self.data.groups_size, self.D))
            phi_var = np.zeros((self.data.groups_size, self.D))
            for d in range(self.D):
