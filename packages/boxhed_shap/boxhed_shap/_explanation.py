...
op_chain_root = OpChain("boxhed_shap.Explanation")
...
class Explanation(metaclass=MetaExplanation):
    """ A slicable set of parallel arrays representing a boxhed_shap explanation.
    """
    def __init__( # pylint: disable=too-many-arguments
        ...
        if output_names is not None and not isinstance(output_names, Alias):
            ...
            if l == 0:
                ...
                raise ValueError("boxhed_shap.Explanation does not yet support output_names of order greater than 3!")
...
class Cohorts():
    ...
    def __repr__(self):
        return f"<boxhed_shap._explanation.Cohorts object with {len(self.cohorts)} cohorts of sizes: {[v.shape for v in self.cohorts.values()]}>"


def _auto_cohorts(shap_values, max_cohorts):
    """ This uses a DecisionTreeRegressor to build a group of cohorts with similar boxhed_shap values.
    """

    # fit a decision tree that well spearates the boxhed_shap values
