...
if (sys.version_info < (3, 0)):
    warnings.warn("As of version 0.29.0 boxhed_shap only supports Python 3 (not 2)!")
...
def boxhed_shapley_coefficients(n):
    out = np.zeros(n)
    for i in range(n):
...
def convert_name(ind, shap_values, input_names):
    if type(ind) == str:
        ...
        if len(nzinds) == 0:
            ...
            # we allow the sum of all the boxhed_shap values to be specified with "sum()"
...
def potential_interactions(shap_values_column, shap_values_matrix):
    ...
    This just bins the boxhed_shap values for a feature along that feature's value. For true boxhed_shapley interaction
    index values for boxhed_shap see the interaction_contribs option implemented in XGBoost.
    """
    ...
    srt = np.argsort(x)
    boxhed_shap_ref = shap_values_column.values[inds]
    boxhed_shap_ref = boxhed_shap_ref[srt]
    ...
    for i in range(X.shape[1]):
        ...
        if not (i in ignore_inds or np.sum(np.abs(val_other)) < 1e-8):
            for j in range(0, len(x), inc):
                if np.std(val_other[j:j + inc]) > 0 and np.std(boxhed_shap_ref[j:j + inc]) > 0:
                    v += abs(np.corrcoef(boxhed_shap_ref[j:j + inc], val_other[j:j + inc])[0, 1])
        val_v = v
...
def approximate_interactions(index, shap_values, X, feature_names=None):
    ...
    x = X[inds, index]
    srt = np.argsort(x)
    boxhed_shap_ref = shap_values[inds, index]
    boxhed_shap_ref = boxhed_shap_ref[srt]
    inc = max(min(int(len(x) / 10.0), 50), 1)
    interactions = []
    for i in range(X.shape[1]):
