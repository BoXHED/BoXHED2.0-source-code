/**
 * Fast recursive computation of boxhed_shap values in trees.
...
/**
 * This runs Tree boxhed_shap with a per tree path conditional dependence assumption.
 */
inline void dense_tree_saabas(tfloat *out_contribs, const TreeEnsemble& trees, const ExplanationDataset &data) {
    ...
    for (unsigned i = 0; i < data.num_X; ++i) {
        ...
        // (this works because of the linearity property of boxhed_shapley values)
        for (unsigned j = 0; j < trees.tree_limit; ++j) {
...
// recursive computation of boxhed_shap values for a decision tree
inline void tree_boxhed_shap_recursive(const unsigned num_outputs, const int *children_left,
...
inline void tree_boxhed_shap(const TreeEnsemble& tree, const ExplanationDataset &data,
...
// Independent Tree boxhed_shap functions below here
// ------------------------------------------
struct Node {
...
inline void tree_boxhed_shap_indep(const unsigned max_depth, const unsigned num_feats,
...
//     ofstream myfile;
//     if (DEBUG) {
//       myfile.open ("/homes/gws/hughchen/boxhed_shap/out.txt",fstream::app);
//       myfile << "Entering tree_boxhed_shap_indep\n";
//     }
    int ns_ctr = 0;
...
/**
 * Runs Tree boxhed_shap with feature independence assumptions on dense data.
 */
...
inline void dense_tree_path_dependent(const TreeEnsemble& trees, const ExplanationDataset &data,
    ...
    for (unsigned i = 0; i < data.num_X; ++i) {
        ...
        for (unsigned j = 0; j < trees.tree_limit; ++j) {
            trees.get_tree(tree, j);
            tree_boxhed_shap(tree, instance, instance_out_contribs, 0, 0);
        }
...
//             self.tree_boxhed_shap(self.trees[t], self._current_X[i,:], self._current_x_missing, phi_diag)
...
//                 self.tree_boxhed_shap(self.trees[t], self._current_X[i,:], self._current_x_missing, phi_on, 1, j)
//                 self.tree_boxhed_shap(self.trees[t], self._current_X[i,:], self._current_x_missing, phi_off, -1, j)
...
inline void dense_tree_interactions_path_dependent(const TreeEnsemble& trees, const ExplanationDataset &data,
    ...
    for (unsigned i = 0; i < data.num_X; ++i) {
        ...
        for (unsigned j = 0; j < trees.tree_limit; ++j) {
            trees.get_tree(tree, j);
            tree_boxhed_shap(tree, instance, diag_contribs, 0, 0);
            ...
            for (unsigned k = 0; k < amount_of_unique_features; ++k) {
                ...
                // compute the boxhed_shap value with this feature held on and off
                std::fill(on_contribs, on_contribs + contrib_row_size, 0);
                std::fill(off_contribs, off_contribs + contrib_row_size, 0);
                tree_boxhed_shap(tree, instance, on_contribs, 1, ind);
                tree_boxhed_shap(tree, instance, off_contribs, -1, ind);
...
/**
 * This runs Tree boxhed_shap with a global path conditional dependence assumption.
 * 
...
inline void dense_global_path_dependent(const TreeEnsemble& trees, const ExplanationDataset &data,
    ...
    for (unsigned i = 0; i < data.num_X; ++i) {
        ...
        tree_boxhed_shap(merged_tree, instance, instance_out_contribs, 0, 0);
...
/**
 * The main method for computing Tree boxhed_shap on models using dense data.
 */
inline void dense_tree_boxhed_shap(const TreeEnsemble& trees, const ExplanationDataset &data, tfloat *out_contribs,
