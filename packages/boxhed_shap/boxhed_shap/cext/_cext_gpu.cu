...
#include "gpu_treeboxhed_shap.h"
#include "tree_boxhed_shap.h"

const float inf = std::numeric_limits<tfloat>::infinity();

struct boxhed_shapSplitCondition {
  boxhed_shapSplitCondition() = default;
  boxhed_shapSplitCondition(tfloat feature_lower_bound, tfloat feature_upper_bound,
  ...
  Merge(const boxhed_shapSplitCondition &other) {  // Combine duplicate features
...
void RecurseTree(
    ...
    std::vector<gpu_treeboxhed_shap::PathElement<boxhed_shapSplitCondition>> *tmp_path,
    std::vector<gpu_treeboxhed_shap::PathElement<boxhed_shapSplitCondition>> *paths,
  ...
  tmp_path->emplace_back(0, tree.features[pos], 0,
                         boxhed_shapSplitCondition{-inf, tree.thresholds[pos], false},
                         left_zero_fraction, 0.0f);

  RecurseTree(left_child, tree, tmp_path, paths, path_idx, num_outputs);

  // Add left split to the path
  tmp_path->back() = gpu_treeboxhed_shap::PathElement<boxhed_shapSplitCondition>(
      0, tree.features[pos], 0,
      boxhed_shapSplitCondition{tree.thresholds[pos], inf, false},
...
std::vector<gpu_treeboxhed_shap::PathElement<boxhed_shapSplitCondition>>
ExtractPaths(const TreeEnsemble &trees) {
  ...
  for (auto i = 0; i < trees.tree_limit; i++) {
    ...
    tmp_path.emplace_back(0, -1, 0, boxhed_shapSplitCondition{-inf, inf, false}, 1.0,
                          0.0f);
...
inline void dense_tree_path_dependent_gpu(
  ...
  thrust::device_vector<float> phis((X.NumCols() + 1) * X.NumRows() *
                                    trees.num_outputs);
  gpu_treeboxhed_shap::GPUTreeboxhed_shap(X, paths.begin(), paths.end(), trees.num_outputs,
                            phis.begin(), phis.end());
  ...
  thrust::for_each(counting, counting + X.NumRows() * trees.num_outputs,
                   [=] __device__(size_t idx) {
                     ...
                     auto phi_idx = gpu_treeboxhed_shap::IndexPhi(
                         row_idx, num_groups, group, X.NumCols(), X.NumCols());
                     d_phis[phi_idx] += d_base_offset[group];
                   });

  // boxhed_shap uses a slightly different layout for multiclass
  ...
  thrust::for_each(
      counting, counting + phis.size(), [=] __device__(size_t idx) {
        ...
        gpu_treeboxhed_shap::FlatIdxToTensorIdx(idx, old_shape, old_idx);
        ...
        size_t transposed_idx =
            gpu_treeboxhed_shap::TensorIdxToFlatIdx(new_shape, new_idx);
...
dense_tree_independent_gpu(const TreeEnsemble &trees,
  ...
  thrust::device_vector<float> phis((X.NumCols() + 1) * X.NumRows() *
                                    trees.num_outputs);
  gpu_treeboxhed_shap::GPUTreeboxhed_shapInterventional(X, R, paths.begin(), paths.end(),
...
inline void dense_tree_path_dependent_interactions_gpu(
  ...
  gpu_treeboxhed_shap::GPUTreeboxhed_shapInteractions(X, paths.begin(), paths.end(),
                                        trees.num_outputs, phis.begin(),
                                        phis.end());
  // Add the base offset term to bias
  thrust::device_vector<double> base_offset(
  ...
  thrust::for_each(counting, counting + X.NumRows() * num_groups,
                   [=] __device__(size_t idx) {
                     size_t row_idx = idx / num_groups;
                     size_t group = idx % num_groups;
                     auto phi_idx = gpu_treeboxhed_shap::IndexPhiInteractions(
                         row_idx, num_groups, group, X.NumCols(), X.NumCols(),
                         X.NumCols());
                     d_phis[phi_idx] += d_base_offset[group];
                   });
...
void dense_tree_boxhed_shap_gpu(const TreeEnsemble &trees,
                         const ExplanationDataset &data, tfloat *out_contribs,
