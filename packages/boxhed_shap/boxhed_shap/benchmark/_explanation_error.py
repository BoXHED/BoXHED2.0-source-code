...
from boxhed_shap.utils import safe_isinstance, MaskedModel, partition_tree_shuffle
from boxhed_shap import Explanation, links
from boxhed_shap.maskers import Text, Image, FixedComposite
...
class ExplanationError():
    ...
    For explanations (like boxhed_shapley values) that explain the difference between one alternative and another
    ...
    are drawn from prefixes of random feature permutations. This seems natural, and aligns with boxhed_shapley value
    ...
    def __init__(self, masker, model, *model_args, batch_size=500, num_permutations=10, link=links.identity, linearize_link=True, seed=38923):
        ...
        Parameters
        ----------
        masker : function or boxhed_shap.Masker
            The masker defines how we hide features during the perturbation process.

        model : function or boxhed_shap.Model
    ...
    def __call__(self, explanation, name, step_fraction=0.01, indices=[], silent=False):
        ...
        if safe_isinstance(explanation, "numpy.ndarray"):
            ...
            raise ValueError("The passed explanation must be either of type numpy.ndarray or boxhed_shap.Explanation!")
