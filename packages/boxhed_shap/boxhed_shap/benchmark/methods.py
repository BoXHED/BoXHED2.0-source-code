...
def linear_boxhed_shap_corr(model, data):
    """ Linear boxhed_shap (corr 1000)
    """
    return LinearExplainer(model, data, feature_dependence="correlation", nsamples=1000).shap_values

def linear_boxhed_shap_ind(model, data):
    """ Linear boxhed_shap (ind)
    """
...
def kernel_boxhed_shap_1000_meanref(model, data):
    """ Kernel boxhed_shap 1000 mean ref.
...
def sampling_boxhed_shap_1000(model, data):
    """ IME 1000
    color = red_blue_circle(0.5)
    linestyle = dashed
    """
...
def tree_boxhed_shap_tree_path_dependent(model, data):
    """ TreeExplainer
...
def tree_boxhed_shap_independent_200(model, data):
...
def mean_abs_tree_boxhed_shap(model, data):
    """ mean(|TreeExplainer|)
...
def deep_boxhed_shap(model, data):
    """ Deep boxhed_shap (DeepLIFT)
    """
