...
linear_regress_methods = [
    ...
    #"kernel_boxhed_shap_100_meanref",
    #"sampling_boxhed_shap_10000",
    "sampling_boxhed_shap_1000",
    "lime_tabular_regression_1000"
    #"sampling_boxhed_shap_100"
]
...
tree_regress_methods = [
    # NEED tree_boxhed_shap_ind
    # NEED split_count?
    "tree_boxhed_shap_tree_path_dependent",
    "tree_boxhed_shap_independent_200",
    "saabas",
    "random",
    "tree_gain",
    "kernel_boxhed_shap_1000_meanref",
...
def run_remote_experiments(experiments, thread_hosts, rate_limit=10):
    ...
    experiments : iterable
        Output of boxhed_shap.benchmark.experiments(...).
    ...
    for host in set(thread_hosts):
        ...
        try:
            subprocess.run(["ssh", hostname, "pkill -f boxhed_shap.benchmark.run_experiment"], timeout=15)
...
def __run_remote_experiment(experiment, remote, cache_dir="/tmp", python_binary="python"):
    ...
    #start = time.time()
    cmd = "CUDA_VISIBLE_DEVICES=\"\" "+python_binary+" -c \"import boxhed_shap; boxhed_shap.benchmark.run_experiment(['%s', '%s', '%s', '%s'], cache_dir='%s')\" &> %s/%s.output" % (
