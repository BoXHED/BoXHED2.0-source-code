...
def test_update():
    ...
    boxhed_shap.benchmark.update(model, X, y, explainer, masker, sort_order, score_function, perturbation, scores)
...
def test_get_benchmark():
    ...
    scores = boxhed_shap.benchmark.get_benchmark(model, X, y, explainer, masker, metrics)
...
def test_get_metrics():
    ...
    assert set(boxhed_shap.benchmark.get_metrics(benchmarks, lambda x, y: x.intersection(y))) == expected_metrics1
    assert set(boxhed_shap.benchmark.get_metrics(benchmarks, lambda x, y: x.union(y))) == expected_metrics2